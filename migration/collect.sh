...
[[ "$(jq -r '.items[0].spec.controllerImage' "$TEMPDIR/linstorcontrollers.json")" == drbd.io/linstor-controller:* ]] || print_row LinstorCluster .spec.controllerImage "Adjust image configuration" "$(jq -r '.items[0].spec.controllerImage' "$TEMPDIR/linstorcontrollers.json")"
...
[ "$(jq -r '.items[0].spec.drbdRepoCred' "$TEMPDIR/linstorcontrollers.json")" == "drbdiocred" ] || print_row LinstorController .spec.drbdRepoCred "Use pull secret to deploy the operator" "$(jq -r '.items[0].spec.drbdRepoCred' "$TEMPDIR/linstorcontrollers.json")"
...
[[ "$(jq -r '.items[0].spec.linstorPluginImage' "$TEMPDIR/linstorcsidrivers.json")" == drbd.io/linstor-csi:* ]] || print_row LinstorCSIDriver .spec.linstorPluginImage "Adjust image configuration" "$(jq -r '.items[0].spec.linstorPluginImage' "$TEMPDIR/linstorcsidrivers.json")"
...
[ "$(jq -r '.items[0].spec.imagePullSecret' "$TEMPDIR/linstorcsidrivers.json")" == "drbdiocred" ] || print_row LinstorCSIDriver .spec.imagePullSecret "Use pull secret to deploy the operator" "$(jq -r '.items[0].spec.imagePullSecret' "$TEMPDIR/linstorcsidrivers.json")"
...
for IDX in $(seq 0 "$(jq '.items | length - 1' "$TEMPDIR/linstorsatellitesets.json")"); do
	...
	[ "$(jq_item -r '.spec.drbdRepoCred')" == "drbdiocred" ] || print_row LinstorSatelliteSet .spec.drbdRepoCred "Use pull secret to deploy the operator" "$(jq_item -r '.spec.drbdRepoCred')"
	...
	[[ "$(jq_item -r '.spec.kernelModuleInjectionImage')" == drbd.io/drbd9-* ]] || print_row LinstorSatelliteSet .spec.kernelModuleInjectionImage "Adjust image configuration" "$(jq_item -r '.spec.kernelModuleInjectionImage')"
	[ "$(jq_item -r '.spec.kernelModuleInjectionMode')" == "ShippedModules" ] || print_row LinstorSatelliteSet .spec.kernelModuleInjectionMode "Adjust image configuration" "$(jq_item -r '.spec.kernelModuleInjectionMode')"
	...
	[[ "$(jq_item -r '.spec.monitoringImage')" == drbd.io/drbd-reactor:* ]] || print_row LinstorSatelliteSet .spec.monitoringImage "Adjust image configuration" "$(jq_item -r '.spec.monitoringImage')"
	...
	[[ "$(jq_item -r '.spec.satelliteImage')" == drbd.io/linstor-satellite:* ]] || print_row LinstorSatelliteSet .spec.satelliteImage "Adjust image configuration" "$(jq_item -r '.spec.satelliteImage')"
